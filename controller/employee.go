...
import (
	...
	"gitlab.com/sirinibin/go-postgresql-rest/models"
	"gitlab.com/sirinibin/go-postgresql-rest/utils"
)
...
func ListEmployee(w http.ResponseWriter, r *http.Request) {
	...
	if ok && len(keys[0]) >= 1 {
		...
		criterias.SortBy = models.ParseString(keys[0])
		err = models.ValidateSortString(criterias.SortBy)
		if err != nil {
			response.Status = false
			response.Errors["sort"] = err.Error()
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(response)
			return
		}
	...
	if ok && len(keys[0]) >= 1 {
		criterias.SearchBy["name"] = models.ParseString(keys[0])
	}
	keys, ok = r.URL.Query()["search[id]"]
	if ok && len(keys[0]) >= 1 {
		criterias.SearchBy["id"] = models.ParseString(keys[0])
	}
	keys, ok = r.URL.Query()["search[email]"]
	if ok && len(keys[0]) >= 1 {
		criterias.SearchBy["email"] = models.ParseString(keys[0])
	}
