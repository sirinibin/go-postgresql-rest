...
import (
	...
	"gitlab.com/sirinibin/go-postgresql-rest/controller"
)

func main() {
	fmt.Println("A GoLang / PostgreSQL Microservice [OAuth2,Redis & JWT used for token management]!")
	...
	go func() {
		log.Fatal(http.ListenAndServeTLS(":2003", "localhost.cert.pem", "localhost.key.pem", router))
	}()
	...
	for _, i := range ifaces {
		...
		for _, addr := range addrs {
			...
			log.Printf("Serving @ https://" + ip.String() + ":2003 /\n")
			log.Printf("Serving @ http://" + ip.String() + ":2002 /\n")
		}
	}
	log.Fatal(http.ListenAndServe(":2002", router))
